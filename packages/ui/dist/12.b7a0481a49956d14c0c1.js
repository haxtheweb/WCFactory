(window.webpackJsonp=window.webpackJsonp||[]).push([[12],{397:function(t,e,i){"use strict";i.r(e);var r=i(277),o=i(324),a=i(284),n=i.n(a),s=i(387),c=i(388),l=i(382),d=i(326),p=i(279),u=i(327),h=i(268),m=i(389),y=i(381),b=new l.a({uri:"http://localhost:4000"}),f=new u.a({uri:"ws://localhost:4000/graphql",options:{reconnect:!0}}),v=Object(p.e)(t=>{var e=t.query,i=Object(h.getMainDefinition)(e),r=i.kind,o=i.operation;return"OperationDefinition"===r&&"subscription"===o},f,b),g=new c.a,w=Object(m.a)({cache:g}),x=new s.a({link:p.a.from([Object(d.a)(t=>{var e=t.graphQLErrors,i=t.networkError;e&&e.map(t=>{var e=t.message,i=t.locations,r=t.path;return console.log(`[GraphQL error]: Message: ${e}, Location: ${i}, Path: ${r}`)}),i&&console.log(`[Network error]: ${i}`)}),new y.a,w,v]),cache:g,connectToDevTools:!1}),$=(i(376),n.a`
  query {
    factories {
      name
      location
...
`);customElements.define("wcfactory-ui-factories",class extends r.a{static get properties(){return{factories:{type:Array}}}constructor(){super(),this.factories=[];try{x.watchQuery({query:$}).subscribe(t=>{var e=t.data.factories;this.factories=e})}catch(t){}}render(){return r.b`<style>:host {
          display: block;
        ...
        #item-desc {
          opacity: 0.7;
        }</style><div id="list">${this.factories.map(t=>r.b`<a id="list-item" href="/factories/${t.name}"><h2 id="item-title">🏭${t.name}</h2><div id="item-desc">${t.location}</div></a>`)}</div>`}});var k=i(345),E=i.n(k),S=n.a`
  query {
    operations {
      script
      location
...
`;customElements.define("wcfactory-ui-scripts",class extends r.a{static get properties(){return{scripts:{type:Array},operations:{type:Array},location:{type:Array},activeScript:{type:String}}}constructor(){super(),this.scripts={},this.operations=[],this.activeScript=""}firstUpdated(){this.subscribeToOperations(),this.subscribeToOperationsChildProcess(),this.fetchOperations()}render(){return r.b`<style>button {
        ...
        #output {
          background: black;
          height: 100px;
        }</style>${this.activeScript} ${this.script} ${this.scripts.map(t=>this.operations.find(e=>e.script===t&&e.location===this.location)?r.b`<span class="script" active="${this.activeScript===t}">🔄${t}</span>`:r.b`<button class="script" @click="${e=>this.runScript(t,this.location)}">🚀${t}</button>`)}<div id="output"></div>`}runScript(t,e){return new Promise(function(i,r){var o=function(){try{return i()}catch(t){return r(t)}},a=function(t){try{return o()}catch(t){return r(t)}};try{return Promise.resolve(x.mutate({mutation:n.a`
