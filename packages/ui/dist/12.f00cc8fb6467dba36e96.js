...
`);customElements.define("wcfactory-ui-factories",class extends r.a{static get properties(){return{factories:{type:Array}}}constructor(){super(),this.factories=[];try{w.watchQuery({query:x}).subscribe(t=>{var e=t.data.factories;this.factories=e})}catch(t){}}render(){return r.b`<style>:host {
          display: block;
        ...
        #item-desc {
          opacity: 0.7;
        }</style><div id="list">${this.factories.map(t=>r.b`<a id="list-item" href="/factories/${t.name}"><h2 id="item-title">🏭${t.name}</h2><div id="item-desc">${t.location}</div></a>`)}</div>`}});var $=i(344),k=i.n($),E=n.a`
  query {
    operations {
      script
      location
...
`;customElements.define("wcfactory-ui-scripts",class extends r.a{static get properties(){return{scripts:{type:Array},operations:{type:Array},location:{type:Array},activeScript:{type:String}}}constructor(){super(),this.scripts={},this.operations=[],this.activeScript=""}firstUpdated(){this.subscribeToOperations(),this.subscribeToOperationsChildProcess(),this.fetchOperations()}render(){return r.b`<style>button {
        ...
        #output {
          background: black;
          height: 100px;
        }</style>${this.activeScript} ${this.script} ${this.scripts.map(t=>this.operations.find(e=>e.script===t&&e.location===this.location)?r.b`<span class="script" active="${this.activeScript===t}">🔄${t}</span>`:r.b`<button class="script" @click="${e=>this.runScript(t,this.location)}">🚀${t}</button>`)}<div id="output"></div>`}runScript(t,e){return new Promise(function(i,r){var o=function(){try{return i()}catch(t){return r(t)}},a=function(t){try{return o()}catch(t){return r(t)}};try{return Promise.resolve(w.mutate({mutation:n.a`
