...
gulp.task('merge', () => {
  return gulp.src('./src/<%= elementName %>.js')
    .pipe(replace(/(template\.innerHTML = `)(`;)/, (match, p1, p2) => {
      const html = fs.readFileSync('./src/<%= elementName %>.html').toString().trim();
      <% if (useSass) { %>
      const cssResult = sass.renderSync({
        file: './src/<%= elementName %>.scss'
      }).css;
      <% } else { %>
      const cssResult = fs.readFileSync('./src/<%= elementName %>.css').toString().trim();
      <% } %>
